...
def update_bed_status(department_id: str, body: BedStatusUpdate):
    ...
    try:
        ...
        from models.models import Bed
        db = SessionLocal()
        ward_label = next(
            (w["ward_label"] for w in layout.values() for b in w["beds"] if b["bed_id"] == body.bed_id),
            None
        )
        if ward_label:
            db_bed = db.query(Bed).filter(Bed.ward == ward_label).first()
            if db_bed:
                db_bed.occupied   = body.new_status == "occupied"
                db_bed.patient_id = body.patient_id if body.new_status == "occupied" else None
                db.commit()
        db.close()
    except Exception as e:
        print(f"[BedPatch] DB persist error: {e}")
