...
export default function NurseDashboard({ onLogout }) {
  ...
  const loadData = useCallback(async () => {
    try {
      // First check backend is reachable with a lightweight call
      const [patientsRes, notifsRes] = await Promise.all([
        workflowApi.getNursePatients(DEMO_NURSE_ID).catch(() => ({ data: { patients: [] } })),
        workflowApi.getNotifications('nurse').catch(() => ({ data: { notifications: [] } })),
      ])
      ...
      // Only mark offline if the request itself failed (network error)
