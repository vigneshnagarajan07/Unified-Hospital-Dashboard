...
const PORTAL_SECTIONS = [
    ...
    { sectionId: 'my-journey', label: 'My Journey',     Icon: Heart         },
]
...
export default function PatientPortal({ onLogout }) {
    ...
    const loadPortalData = async () => {
        try {
            ...
            setBillData(billRes.data?.bill_estimate || billRes.data)
        ...
        <div className="min-h-screen bg-slate-50 flex flex-col">
            ...
            <div className="flex flex-1 max-w-screen-2xl mx-auto w-full">
                ...
                <main className="flex-1 px-6 py-6">
                    ...
                    {/* Status banners */}
                    {isLoading && (
                        <div className="mb-6 flex items-center gap-3 bg-sky-50 border border-sky-200 rounded-2xl px-5 py-3">
                            <RefreshCw size={14} className="text-rose-500 animate-spin shrink-0" />
                            <p className="text-sm font-semibold text-rose-700">Loading your health record...</p>
                        </div>
                    )}
                    {fetchError && !isLoading && (
                        <div className="mb-6 flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4">
                            ...
                            <button onClick={loadPortalData} className="ml-auto px-3 py-1.5 rounded-xl bg-amber-100 text-amber-700 text-xs font-bold hover:bg-amber-200 border border-amber-300 shrink-0">Retry</button>
                        </div>
                    )}
                    ...
                    {/* ── Section 8 : My Care Journey ── */}
                    <PatientSection
                        sectionId="my-journey"
                        title="My Care Journey"
                        subtitle="Pharmacy orders and billing for your admission"
                    >
                        <WorkflowPanel role="patient" patientId={DEMO_PATIENT_ID} />
                    </PatientSection>
