  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [ordersRes, patientsRes, billingRes, notifsRes] = await Promise.all([
        workflowApi.getPharmacyQueue().catch(() => ({ data: { orders: [] } })),
        workflowApi.getWorkflowPatients().catch(() => ({ data: { patients: [] } })),
        workflowApi.getBillingSummary().catch(() => ({ data: {} })),
        workflowApi.getNotifications('pharmacy').catch(() => ({ data: { notifications: [] } })),
      ])
